...
import { 
  ...
  Briefcase, Send, MapPin, Phone, Calendar, Heart, FileText, RefreshCw, QrCode, ShieldCheck
} from "lucide-react";
...
function PedidoContent() {
  ...
  const validarReceita = async (isManual = true) => {
    ...
    try {
      ...
      if (response.ok && data && !data.error) {
        setApiData(data); 
        const nomeApi = data.nome || data.response?.content?.nome?.conteudo?.nome || "";
        ...
        if (response.status === 429) {
            ...
            if (isManual) alert(`❌ ${data.error || "Erro ao buscar dados"}`);
        }
      }
    } catch (error) {
  ...
  // Atalhos de Dados
  const situacaoReceita = apiData?.situacao || apiData?.response?.content?.nome?.conteudo?.situacao_receita || "PENDENTE";
  ...
  // --- FUNÇÃO ATUALIZADA: SALVAR COM NOME DO VENDEDOR ---
  const handleFinalizarSolicitacao = async () => {
    ...
    try {
        ...
        // 2. Busca o NOME do vendedor na tabela profiles
        let nomeVendedor = user.email; // Fallback se não achar perfil
        ...
        // 3. Monta o payload com o nome do vendedor
        const payload = {
            seller_id: user.id, 
            seller_name: nomeVendedor, // <--- ADICIONADO AQUI
  ...
  if (loadingValidacao) {
      return (
          <div className="min-h-screen bg-zinc-900 flex flex-col items-center justify-center gap-4">
              ...
              <p className="text-zinc-400 font-bold uppercase tracking-widest text-xs animate-pulse">Gerando Documento...</p>
          </div>
      )
  }
    ...
    <div className="min-h-screen bg-zinc-100 font-sans text-zinc-900 pb-32 md:pb-20 print:bg-white print:p-0">
        ...
        <header className="px-4 md:px-6 py-4 bg-zinc-900 border-b border-zinc-800 sticky top-0 z-50 print:hidden shadow-xl safe-area-top">
            <div className="max-w-5xl mx-auto flex items-center justify-between">
                <button onClick={() => router.back()} className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors font-bold text-xs uppercase tracking-widest">
                    <ArrowLeft size={18}/> <span className="hidden md:inline">Voltar</span>
                </button>
                <div className="flex items-center gap-2 md:gap-3">
                    <button onClick={() => window.print()} className="bg-zinc-800 border border-zinc-700 text-zinc-300 p-2 md:px-4 md:py-2 rounded-lg text-xs font-bold uppercase hover:bg-zinc-700 hover:text-white flex items-center gap-2 shadow-sm transition-all">
            ...
            <div className="bg-white shadow-lg md:shadow-2xl rounded-2xl md:rounded-none overflow-hidden w-full min-h-[80vh] md:min-h-[297mm] flex flex-col relative print:shadow-none print:w-full">
                    ...
                    <div className="flex flex-col gap-1 w-full md:w-auto">
                        <div className="flex justify-between items-center md:block">
                            ...
                            <div className="md:hidden">
                                ...
                                <span className="text-sm font-mono font-black text-zinc-900 bg-zinc-100 px-2 py-1 rounded">#{numeroPedido}</span>
                            </div>
                        </div>
                ...
                <div className="p-6 md:p-10 space-y-8 flex-1 relative z-10">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-6 gap-y-4 text-xs">
                            ...
                            <div className="md:col-span-1">
                                ...
                                <div className="font-mono font-bold text-base md:text-lg border-b border-dotted border-zinc-300 pb-1 flex items-center gap-2">
                                    {dados.cpf}
                                    {situacaoReceita === 'REGULAR' && <ShieldCheck size={14} className="text-green-600 print:hidden flex-shrink-0"/>}
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-1">
                                ...
                                <div className={`font-black uppercase text-[10px] px-2 py-0.5 rounded w-fit ${situacaoReceita === 'REGULAR' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                    {situacaoReceita}
                                </div>
                            </div>
                    ...
                    <div className="border-2 border-dashed border-zinc-300 rounded-lg p-4 min-h-[100px] md:min-h-[120px] bg-zinc-50/50 print:bg-white relative">
                        <p className="absolute top-2 left-3 text-[9px] font-bold text-zinc-400 uppercase bg-white px-1">Observações / Acessórios</p>
                    </div>
                ...
                <div className="mt-auto p-6 md:p-10 pt-0">
                    ...
                    <div className="text-center mt-4 pt-2 border-t border-zinc-100">
                        <p className="text-[9px] font-bold text-zinc-300 uppercase tracking-widest">Belém, {dataAtual} • WBCNAC.com</p>
                    </div>
                </div>
