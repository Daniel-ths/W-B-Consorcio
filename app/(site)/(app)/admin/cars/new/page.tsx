        ...
        v.exterior_colors?.forEach((c: any) => {
          ...
          if (c.images) {
            Object.keys(c.images).forEach((k) => {
              if (c.images[k]) images.push({ url: c.images[k], label: `Cor: ${c.name} (${k})` });
            });
          }
        });

        v.wheels?.forEach((w: any) => {
          if (w.image) images.push({ url: w.image, label: `Roda: ${w.name}` });
        });

        v.accessories?.forEach((a: any) => {
          if (a.image) images.push({ url: a.image, label: `Aces: ${a.name}` });
        });
            ...
            {recentImages.length > 0 && (
              ...
              >
                <span className="flex items-center gap-2">
                  <CheckCircle2 size={14} /> Recentes ({recentImages.length})
                </span>
...
const MiniUpload = ({ label, previewUrl, onRemove, onSelect }: UploadProps) => {
  return (
    <div className="w-full">
      <label className="block text-[9px] font-bold uppercase text-gray-400 mb-1 truncate" title={label}>
        {label}
      </label>
      <div
        className={`relative h-20 rounded-lg border-2 border-dashed transition-all flex flex-col items-center justify-center overflow-hidden group 
      ...
      >
        ...
        {previewUrl ? (
          <>
            ...
            <div className="absolute top-1 right-1 z-30">
              <button
                ...
                className="bg-red-500 text-white p-1 rounded-full shadow hover:scale-110 z-40 cursor-pointer"
              >
                <X size={10} />
              </button>
            </div>
          ...
          <div className="text-center p-1">
            <Plus className="mx-auto h-5 w-5 text-gray-300" />
          </div>
        )}
      </div>
    </div>
...
const ImageUpload = ({ label, previewUrl, setFile, onOpenLibrary }: UploadProps) => {
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile && setFile) setFile(selectedFile, URL.createObjectURL(selectedFile));
    e.target.value = "";
  };
  ...
  return (
    <div className="w-full">
      ...
      <div
        className={`relative h-32 rounded-lg border-2 border-dashed transition-all flex flex-col items-center justify-center overflow-hidden group 
          ...
          <>
            ...
            <div className="absolute top-2 right-2 z-30">
              <button
                ...
                className="bg-red-500 text-white p-1.5 rounded-full shadow hover:scale-110 z-40 cursor-pointer"
              >
                <Trash2 size={14} />
              </button>
            </div>
...
const ColorList = ({ colors, setColors, formatMoneyInput, preventSubmit, onOpenLibrary }: any) => {
  const addColor = () =>
    setColors([
      ...colors,
      { id: crypto.randomUUID(), name: "", hex: "#000000", priceFormatted: "R$ 0,00", files: {}, previews: {} },
    ]);
  ...
  const updatePrice = (i: number, val: string) => {
    const n = [...colors];
    n[i].priceFormatted = formatMoneyInput(val);
    setColors(n);
  };
  const updateHex = (i: number, val: string) => {
    const n = [...colors];
    n[i].hex = val;
    setColors(n);
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button type="button" onClick={addColor} className="text-[10px] font-bold bg-black text-white px-3 py-1.5 rounded">
          + Adicionar Cor
        </button>
      </div>
        ...
        {colors.map((c: ColorItem, i: number) => (
          <div key={c.id} className="border-b border-gray-100 pb-6 last:border-0 bg-gray-50/50 p-4 rounded-xl">
            <div className="flex gap-3 items-end mb-4">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="border rounded h-9 px-2 w-full text-sm"
                  value={c.name}
                  onChange={(e) => updateName(i, e.target.value)}
                  placeholder="Ex: Branco Summit"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="color"
                  className="border rounded h-9 w-10 p-0.5 cursor-pointer"
                  value={c.hex}
                  onChange={(e) => updateHex(i, e.target.value)}
                />
              </div>

              <button type="button" onClick={() => removeColor(i)} className="mb-1 text-red-500 hover:bg-red-50 p-2 rounded">
                <Trash2 size={16} />
              </button>
            </div>
            ...
            <div className="flex justify-end mb-2">
              <button
                type="button"
                onClick={() => onOpenLibrary((url: string) => updateColorFile(i, "front", null, url))}
...
const ConfigurableList = ({ configurables, setConfigurables, formatMoneyInput, preventSubmit, onOpenLibrary }: any) => {
  const addItem = () =>
    setConfigurables([
      ...configurables,
      { id: crypto.randomUUID(), name: "", priceFormatted: "R$ 0,00", type: "acc_ext", file: null, preview: null },
    ]);

  const updateType = (i: number, val: string) => {
    const n = [...configurables];
    n[i].type = val;
    setConfigurables(n);
  };
  const updateName = (i: number, val: string) => {
  ...
  const updateFile = (i: number, file: File | null, url: string | null) => {
    const n = [...configurables];
    n[i].file = file;
    n[i].preview = url;
    setConfigurables(n);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex justify-between mb-4">
        <h2 className="text-sm font-bold uppercase text-gray-800 flex items-center gap-2">
          <Settings size={16} /> Itens de Configuração
        </h2>
        <button type="button" onClick={addItem} className="text-xs font-bold bg-black text-white px-3 py-1.5 rounded">
          + Adicionar
        </button>
      </div>

      <div className="space-y-4">
        {configurables.map((item: ConfigurableItem, i: number) => (
          <div key={item.id} className="flex flex-col md:flex-row gap-3 items-start border-b border-gray-100 pb-4 bg-gray-50/50 p-3 rounded-xl">
            <div className="w-full md:w-40">
              ...
              <select className="w-full h-9 px-2 text-xs border rounded bg-white" value={item.type} onChange={(e) => updateType(i, e.target.value)}>
                <option value="wheel">Roda</option>
                <option value="seat">Banco</option>
                <option value="acc_ext">Aces. Externo</option>
                <option value="acc_int">Aces. Interno</option>
              </select>
            </div>

            <div className="flex-1 grid grid-cols-2 gap-2 w-full">
              <div>
                <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Nome</label>
                <input onKeyDown={preventSubmit} className="border rounded h-9 px-2 w-full text-sm" value={item.name} onChange={(e) => updateName(i, e.target.value)} />
              </div>
              <div>
                <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Preço (+)</label>
                <input onKeyDown={preventSubmit} className="border rounded h-9 px-2 w-full text-sm" value={item.priceFormatted} onChange={(e) => updatePrice(i, e.target.value)} />
              </div>
            </div>
            ...
            <button type="button" onClick={() => removeItem(i)} className="mt-5 text-red-500 hover:bg-red-50 p-2 rounded">
              <Trash2 size={16} />
            </button>
          </div>
        ))}
...
// ✅ GERENCIADOR GLOBAL DE ACESSÓRIOS (aba separada)
const AccessoriesManager = ({
  ...
  preventSubmit: (e: React.KeyboardEvent) => void;
  onOpenLibrary: (cb: (url: string) => void) => void;
  uploadToSupabase: (file: File | null, path: string, existingUrl: string | null) => Promise<string | null>;
  onRefreshVehicles: () => Promise<void> | void;
}) => {
  const [items, setItems] = useState<GlobalAccessoryItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [savingId, setSavingId] = useState<string | null>(null);

  useEffect(() => {
    const map = new Map<string, { item: GlobalAccessoryItem; vehiclesSet: Set<number> }>();

    vehicles.forEach((v) => {
      (v.accessories || []).forEach((a: any) => {
        const stableId =
          a?.id ||
          `${a?.type || "unknown"}::${(a?.name || "").trim().toLowerCase()}::${a?.image || ""}`;

        const type: "exterior" | "interior" = a?.type === "interior" ? "interior" : "exterior";
        ...
        if (!map.has(stableId)) {
          map.set(stableId, {
            ...
            vehiclesSet: new Set<number>(),
          });
        }

        map.get(stableId)!.vehiclesSet.add(v.id);
      });
    });

    const arr = Array.from(map.values()).map(({ item, vehiclesSet }) => ({
      ...item,
      vehiclesCount: vehiclesSet.size,
    }));

    arr.sort((a, b) => (b.vehiclesCount - a.vehiclesCount) || a.name.localeCompare(b.name));
    setItems(arr);
  }, [vehicles]);

  const updateField = (id: string, patch: Partial<GlobalAccessoryItem>) => {
    setItems((prev) => prev.map((it) => (it.id === id ? { ...it, ...patch } : it)));
  };

  const saveAccessory = async (acc: GlobalAccessoryItem) => {
    setSavingId(acc.id);
    try {
      let finalImage = acc.preview;

      if (acc.file) {
        const safeName = (acc.name || "acessorio")
          .toLowerCase()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .replace(/[^a-z0-9]+/g, "-")
        ...
        finalImage = await uploadToSupabase(acc.file, `accessories/global/${acc.type}/${safeName}`, acc.preview);
      }

      const updated = {
        name: acc.name,
        type: acc.type,
        price: parseMoney(acc.priceFormatted),
        image: finalImage,
      };
      ...
      for (const v of vehicles) {
        const oldList = Array.isArray(v.accessories) ? v.accessories : [];
        if (!oldList.length) continue;

        let touched = false;

        const newList = oldList.map((a: any) => {
          const stableId =
            a?.id ||
            `${a?.type || "unknown"}::${(a?.name || "").trim().toLowerCase()}::${a?.image || ""}`;

          if (stableId !== acc.id) return a;

          touched = true;
          const keepId = a?.id || undefined;
        ...
        const { error } = await supabase.from("vehicles").update({ accessories: newList }).eq("id", v.id);
        if (error) throw error;
      }

      updateField(acc.id, { file: null, preview: updated.image || null });
      await onRefreshVehicles();
      alert("Acessório atualizado em todos os veículos!");
    } catch (e: any) {
      alert("Erro ao salvar acessório: " + (e?.message || "desconhecido"));
    } finally {
      setLoading(false);
      setSavingId(null);
    }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-bold uppercase text-gray-800 flex items-center gap-2">
            <Settings size={16} /> Acessórios Cadastrados (Global)
          </h2>
          <p className="text-xs text-gray-500 mt-1">Edite aqui e aplique automaticamente nos veículos que usam o acessório.</p>
        </div>
        {loading && (
          <div className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded flex items-center gap-2">
            <Loader2 className="animate-spin" size={14} /> Atualizando veículos...
          </div>
        )}
      </div>

      {items.length === 0 ? (
        <div className="text-sm text-gray-500">Nenhum acessório encontrado nos veículos ainda.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {items.map((acc) => (
            <div key={acc.id} className="border border-gray-200 rounded-xl p-4 bg-gray-50/40">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-bold uppercase text-gray-500 bg-white border px-2 py-1 rounded">
                      {acc.type === "exterior" ? "Exterior" : "Interior"}
                    </span>
                    <span className="text-[10px] font-bold uppercase text-gray-500">Usado em {acc.vehiclesCount} veículo(s)</span>
                  </div>

                  <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Nome</label>
                  <input
                    onKeyDown={preventSubmit}
                    className="border rounded h-9 px-2 w-full text-sm bg-white"
                    value={acc.name}
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    <div>
                      <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Tipo</label>
                      <select
                        className="border rounded h-9 px-2 w-full text-xs bg-white"
                        value={acc.type}
                        onChange={(e) => updateField(acc.id, { type: e.target.value as "exterior" | "interior" })}
                      >
                    ...
                    <div>
                      ...
                      <input
                        onKeyDown={preventSubmit}
                        className="border rounded h-9 px-2 w-full text-sm bg-white"
                        value={acc.priceFormatted}
                        onChange={(e) => updateField(acc.id, { priceFormatted: formatMoneyInput(e.target.value) })}
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="w-24">
                  <ImageUpload
                    label="Foto"
                    previewUrl={acc.preview}
                    setFile={(f, u) => updateField(acc.id, { file: f || null, preview: u || null })}
                    onOpenLibrary={() => onOpenLibrary((url: string) => updateField(acc.id, { file: null, preview: url }))}
                  />
                </div>
              </div>
              ...
              <div className="mt-3 flex justify-end">
                <button
                  type="button"
                  onClick={() => saveAccessory(acc)}
                  disabled={loading || savingId === acc.id}
                  className="px-4 py-2 rounded-lg bg-black text-white text-xs font-bold uppercase hover:bg-gray-800 disabled:opacity-60 flex items-center gap-2"
                >
                  {savingId === acc.id ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}
                  {savingId === acc.id ? "Salvando..." : "Salvar"}
                </button>
              </div>
            </div>
          ))}
...
// ✅ GERENCIADOR GLOBAL DE RODAS (aba separada)
const WheelsManager = ({
  vehicles,
  formatMoneyInput,
  parseMoney,
  ...
  const [items, setItems] = useState<GlobalWheelItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [savingId, setSavingId] = useState<string | null>(null);

  useEffect(() => {
    const map = new Map<string, { item: GlobalWheelItem; vehiclesSet: Set<number> }>();

    vehicles.forEach((v) => {
      (v.wheels || []).forEach((w: any) => {
        const stableId = w?.id || `wheel::${(w?.name || "").trim().toLowerCase()}::${w?.image || ""}`;
        ...
        if (!map.has(stableId)) {
          map.set(stableId, {
            item: {
              id: stableId,
              name: w?.name || "",
              priceFormatted: formatMoneyInput(((w?.price || 0).toString()) + "00"),
              preview: w?.image || null,
              file: null,
              vehiclesCount: 0,
            },
            vehiclesSet: new Set<number>(),
    ...
    const arr = Array.from(map.values()).map(({ item, vehiclesSet }) => ({ ...item, vehiclesCount: vehiclesSet.size }));
    arr.sort((a, b) => (b.vehiclesCount - a.vehiclesCount) || a.name.localeCompare(b.name));
    setItems(arr);
  }, [vehicles]);

  const updateField = (id: string, patch: Partial<GlobalWheelItem>) => {
    setItems((prev) => prev.map((it) => (it.id === id ? { ...it, ...patch } : it)));
  };

  const saveWheel = async (wheel: GlobalWheelItem) => {
    setSavingId(wheel.id);
    try {
      let finalImage = wheel.preview;

      if (wheel.file) {
        const safeName = (wheel.name || "roda")
          .toLowerCase()
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .replace(/[^a-z0-9]+/g, "-")
        ...
        finalImage = await uploadToSupabase(wheel.file, `wheels/global/${safeName}`, wheel.preview);
      }

      const updated = {
        name: wheel.name,
        price: parseMoney(wheel.priceFormatted),
        image: finalImage,
      };
      ...
      for (const v of vehicles) {
        const oldList = Array.isArray(v.wheels) ? v.wheels : [];
        if (!oldList.length) continue;

        let touched = false;

        const newList = oldList.map((w: any) => {
          const stableId = w?.id || `wheel::${(w?.name || "").trim().toLowerCase()}::${w?.image || ""}`;
          if (stableId !== wheel.id) return w;

          touched = true;
          const keepId = w?.id || undefined;
        ...
        const { error } = await supabase.from("vehicles").update({ wheels: newList }).eq("id", v.id);
        if (error) throw error;
      }

      updateField(wheel.id, { file: null, preview: updated.image || null });
      await onRefreshVehicles();
      alert("Roda atualizada em todos os veículos!");
    } catch (e: any) {
      alert("Erro ao salvar roda: " + (e?.message || "desconhecido"));
    } finally {
      setLoading(false);
      setSavingId(null);
    }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-sm font-bold uppercase text-gray-800 flex items-center gap-2">
            <Settings size={16} /> Rodas Cadastradas (Global)
          </h2>
          <p className="text-xs text-gray-500 mt-1">Edite aqui e aplique automaticamente nos veículos que usam a roda.</p>
        </div>
        {loading && (
          <div className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded flex items-center gap-2">
            <Loader2 className="animate-spin" size={14} /> Atualizando veículos...
      ...
      {items.length === 0 ? (
        <div className="text-sm text-gray-500">Nenhuma roda encontrada nos veículos ainda.</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {items.map((wheel) => (
            <div key={wheel.id} className="border border-gray-200 rounded-xl p-4 bg-gray-50/40">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[10px] font-bold uppercase text-gray-500">Usado em {wheel.vehiclesCount} veículo(s)</span>
                  </div>
                  ...
                  <input
                    ...
                    className="border rounded h-9 px-2 w-full text-sm bg-white"
                    value={wheel.name}
                    onChange={(e) => updateField(wheel.id, { name: e.target.value })}
                  />
                  ...
                  <div className="mt-3">
                    ...
                    <input
                      onKeyDown={preventSubmit}
                      className="border rounded h-9 px-2 w-full text-sm bg-white"
                      value={wheel.priceFormatted}
                      onChange={(e) => updateField(wheel.id, { priceFormatted: formatMoneyInput(e.target.value) })}
                    />
                  </div>
                </div>
                ...
                <div className="w-24">
                  <ImageUpload
                    label="Foto"
                    previewUrl={wheel.preview}
                    setFile={(f, u) => updateField(wheel.id, { file: f || null, preview: u || null })}
                    onOpenLibrary={() => onOpenLibrary((url: string) => updateField(wheel.id, { file: null, preview: url }))}
                  />
                </div>
              </div>
              ...
              <div className="mt-3 flex justify-end">
                <button
                  type="button"
                  onClick={() => saveWheel(wheel)}
                  disabled={loading || savingId === wheel.id}
                  className="px-4 py-2 rounded-lg bg-black text-white text-xs font-bold uppercase hover:bg-gray-800 disabled:opacity-60 flex items-center gap-2"
                >
                  {savingId === wheel.id ? <Loader2 className="animate-spin" size={14} /> : <Save size={14} />}
                  {savingId === wheel.id ? "Salvando..." : "Salvar"}
                </button>
              </div>
            </div>
          ))}
...
async function fetchVehicles() {
  ...
  const handleGalleryBatchUpload = async (files: FileList) => {
    ...
    try {
      ...
      const basePath = `${catSlug}/${vehicleSlug}/gallery`;
  ...
  // ======= VEÍCULOS (estoque + editor) =======
  const groupedVehicles = useMemo(() => {
    return vehicleList.reduce((acc: any, vehicle) => {
  ...
  const selectAllInCategory = (categoryName: string) => {
    const list = groupedVehicles[categoryName] || [];
    const idsInCategory = list.map((v: any) => v.id);

    const allSelected = idsInCategory.length > 0 && idsInCategory.every((id: number) => selectedIds.includes(id));

    if (allSelected) {
      setSelectedIds((prev) => prev.filter((id) => !idsInCategory.includes(id)));
    } else {
      setSelectedIds((prev) => {
        const next = [...prev];
  ...
  const handleNameChange = (e: any) => {
    ...
    if (!editingId) {
      const slug = name
        ...
        .replace(/[^a-z0-9 ]/g, "")
        .replace(/\s+/g, "-");
      setFormData((prev) => ({ ...prev, model_name: name, slug }));
    } else {
  ...
  const startEditing = (v: any) => {
    ...
    // ✅ FIX: preserva id original se existir
    ...
    if (v.seat_types) {
      merged = merged.concat(
        v.seat_types.map((s: any) => ({
          ...s,
          id: s.id || crypto.randomUUID(),
          type: "seat",
          file: null,
          preview: s.image,
          priceFormatted: formatMoneyInput((s.price || 0).toString() + "00"),
        }))
      );
    }
  ...
  const cancelEditing = () => {
    ...
    setDashImg({ file: null, url: null });
    setDashDesc("");
    setSeatsImg({ file: null, url: null });
    setSeatsDesc("");
    setColors([{ id: crypto.randomUUID(), name: "Padrão", hex: "#000000", priceFormatted: "R$ 0,00", files: {}, previews: {} }]);
    setConfigurables([]);
  };
  ...
  const handleVehicleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const col of colors) {
        ...
        finalColors.push({ name: col.name, hex: col.hex, price: parseMoney(col.priceFormatted), image: images.front, images });
      }

      const finalWheels: any[] = [];
      const finalSeatTypes: any[] = [];
...
const { error } = await supabase
  ...
  const handleBannerSubmit = async (e: React.FormEvent) => {
    ...
    setBLoading(true);
    try {
      const imageUrl = await uploadToSupabase(bImage.file, "banners", null);
      await supabase.from("hero_slides").insert({ title: bFormData.title, subtitle: bFormData.subtitle, image_url: imageUrl });
    ...
    <div className="min-h-screen bg-gray-50 pt-24 pb-32 px-4 font-sans relative">
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col items-center mb-10">
          <img src="https://qkpfsisyaohpdetyhtjd.supabase.co/storage/v1/object/public/cars/parceirologo.png" className="h-12 mb-4" />
          <h1 className="text-2xl font-bold text-gray-900 mb-6">Painel Administrativo</h1>

          {/* ✅ TABS ORGANIZADAS */}
          <div className="bg-white p-1 rounded-full shadow-sm border border-gray-200 inline-flex flex-wrap justify-center gap-1">
            <button
              onClick={() => setActiveTab("vehicles")}
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                activeTab === "vehicles" ? "bg-black text-white shadow" : "text-gray-500 hover:text-black"
              }`}
            >
              Veículos
            </button>
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                activeTab === "accessories" ? "bg-black text-white shadow" : "text-gray-500 hover:text-black"
              }`}
            >
              Acessórios
            </button>
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                activeTab === "wheels" ? "bg-black text-white shadow" : "text-gray-500 hover:text-black"
              }`}
            >
              Rodas
            </button>
            ...
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${
                activeTab === "banners" ? "bg-black text-white shadow" : "text-gray-500 hover:text-black"
              }`}
            >
              Banners
            </button>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {(groupedVehicles[categoryName] || []).map((v: any) => {
                      ...
                      return (
                        <div
                          key={v.id}
                          className={`p-4 rounded-xl border flex gap-4 items-center transition-all relative group ${
                            isSelected ? "bg-blue-50 border-blue-500 ring-1 ring-blue-500" : "bg-white border-gray-200 shadow-sm hover:border-gray-300"
                          } ${v.is_visible === false ? "opacity-60" : ""}`}
                        >
                          <div onClick={() => toggleSelectVehicle(v.id)} className="cursor-pointer">
                            {isSelected ? <CheckSquare className="text-blue-600" size={20} /> : <Square className="text-gray-300 hover:text-gray-500" size={20} />}
                          </div>

                          {v.is_visible === false && (
                            <div className="absolute top-2 right-2 bg-red-100 text-red-600 px-2 py-0.5 rounded text-[9px] font-bold flex items-center gap-1 uppercase tracking-wider">
                              <EyeOff size={10} /> Oculto
                            </div>
                          )}

                          <div className="w-20 h-20 bg-white rounded-lg shrink-0 overflow-hidden border border-gray-100">
                            <img src={v.image_url} className="w-full h-full object-contain" />
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[10px] font-bold uppercase text-gray-400">
                              {v.transmission_type === "automatic" ? "Automático" : v.transmission_type === "manual" ? "Manual" : "Ambos"}
                            </span>
                          </div>

                          <div className="flex gap-1">
                            <Link href={`/configurador?id=${v.id}`} target="_blank" className="p-2 text-green-600 hover:bg-green-50 rounded-full">
                              <Eye size={18} />
                            </Link>
                            <button onClick={() => startEditing(v)} className="p-2 text-blue-500 hover:bg-blue-100 rounded-full">
                              <Pencil size={18} />
                            </button>
                            <button onClick={() => deleteVehicle(v.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-full">
                              <Trash2 size={18} />
                            </button>
                          </div>
                        </div>
              ...
              <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 p-4 shadow-[0_-5px_20px_rgba(0,0,0,0.1)] z-50 flex justify-center animate-in slide-in-from-bottom-10">
                <div className="max-w-5xl w-full flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-gray-900">{selectedIds.length} selecionados</span>
                    <button onClick={() => setSelectedIds([])} className="text-xs text-red-500 hover:underline">
                      Limpar
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleBatchUpdate("automatic")} disabled={batchLoading} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg text-xs font-bold uppercase transition-colors">
                      Automático
                    </button>
                    <button onClick={() => handleBatchUpdate("manual")} disabled={batchLoading} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg text-xs font-bold uppercase transition-colors">
                      Manual
                    </button>
                    <button onClick={() => handleBatchUpdate("both")} disabled={batchLoading} className="px-4 py-2 bg-black text-white rounded-lg text-xs font-bold uppercase hover:bg-gray-800 transition-colors">
                      Ambos
                    </button>
                  </div>
                </div>
            ...
            <form onSubmit={handleVehicleSubmit} className={`space-y-6 relative ${editingId ? "p-6 bg-blue-50/50 rounded-3xl border border-blue-100" : ""}`}>
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                  {editingId ? <Pencil className="bg-blue-600 text-white rounded-full p-1.5 w-7 h-7" /> : <Plus className="bg-black text-white rounded-full p-1 w-6 h-6" />}
                  {editingId ? "Editando" : "Novo Veículo"}
                </h2>
                {editingId && (
                  <button type="button" onClick={cancelEditing} className="text-xs font-bold text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg flex items-center gap-1 transition">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <div className="md:col-span-2">
                    ...
                    <input onKeyDown={preventSubmit} value={formData.model_name} className="w-full mt-1 h-10 px-3 border rounded focus:ring-1 focus:ring-black outline-none" onChange={handleNameChange} required />
                  </div>
                  ...
                  <div className="flex items-center pt-5">
                    <button
                      ...
                      className={`flex items-center gap-3 px-4 py-2 rounded-lg border transition-all w-full ${isVisible ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}
                    >
                      <div className={`w-5 h-5 rounded-full flex items-center justify-center ${isVisible ? "bg-green-600 text-white" : "bg-red-600 text-white"}`}>
                        {isVisible ? <CheckCircle2 size={12} /> : <EyeOff size={12} />}
                      </div>
                  ...
                  <div>
                    ...
                    <select value={formData.category_id} className="w-full mt-1 h-10 px-3 border rounded bg-white" onChange={(e) => setFormData({ ...formData, category_id: e.target.value })} required>
                  ...
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase">Preço Base</label>
                    <input onKeyDown={preventSubmit} value={price} className="w-full mt-1 h-10 px-3 border rounded" onChange={(e) => setPrice(formatMoneyInput(e.target.value))} required />
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    <ImageUpload label="1. Painel" previewUrl={dashImg.url} setFile={(f, u) => setDashImg({ file: f, url: u })} />
                    <input onKeyDown={preventSubmit} value={dashDesc} onChange={(e) => setDashDesc(e.target.value)} className="w-full mt-2 border rounded h-9 px-2 text-xs" placeholder="Descrição do Painel" />
                  </div>
                  <div>
                    <ImageUpload label="2. Bancos Padrão" previewUrl={seatsImg.url} setFile={(f, u) => setSeatsImg({ file: f, url: u })} />
                    <input onKeyDown={preventSubmit} value={seatsDesc} onChange={(e) => setSeatsDesc(e.target.value)} className="w-full mt-2 border rounded h-9 px-2 text-xs" placeholder="Descrição dos Bancos" />
                  </div>
                </div>
              </div>

              <ColorList colors={colors} setColors={setColors} formatMoneyInput={formatMoneyInput} preventSubmit={preventSubmit} onOpenLibrary={openLibraryFor} />

              <ConfigurableList configurables={configurables} setConfigurables={setConfigurables} formatMoneyInput={formatMoneyInput} preventSubmit={preventSubmit} onOpenLibrary={openLibraryFor} />
            ...
            <form onSubmit={handleBannerSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    onKeyDown={preventSubmit}
                    className="w-full mt-1 h-10 px-3 text-sm border rounded"
                    value={bFormData.title}
                    onChange={(e) => setBFormData({ ...bFormData, title: e.target.value })}
                    required
                  />
                </div>
                <div>
                  ...
                  <input
                    onKeyDown={preventSubmit}
                    className="w-full mt-1 h-10 px-3 text-sm border rounded"
                    value={bFormData.subtitle}
                    onChange={(e) => setBFormData({ ...bFormData, subtitle: e.target.value })}
                    required
                  />
                </div>
                <button disabled={bLoading} type="submit" className="w-full h-10 bg-black text-white font-bold uppercase text-xs rounded hover:bg-gray-800 mt-2">
                  {bLoading ? "..." : "Adicionar Banner"}
                </button>
              </div>

              <div>
                <ImageUpload label="Imagem (1920x1080)" previewUrl={bImage.url} setFile={(f, u) => setBImage({ file: f, url: u })} />
              </div>
            </form>
            ...
            <div className="grid grid-cols-1 gap-4 mt-6">
              {banners.map((banner) => (
                <div key={banner.id} className="bg-white p-3 rounded-xl border border-gray-200 flex items-center gap-4 shadow-sm">
                  ...
                  <button onClick={() => deleteBanner(banner.id)} className="p-2 text-red-500">
                    <Trash2 size={18} />
                  </button>
                </div>
              ))}
