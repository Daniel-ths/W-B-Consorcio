...
export default function AdminDashboard() {
  ...
  const [isAuthorized, setIsAuthorized] = useState(false); // O CADEADO DE SEGURANÇA 🔒

  const [searchTerm, setSearchTerm] = useState("");
  const [isDeleting, setIsDeleting] = useState<string | null>(null);
  const [isUpdating, setIsUpdating] = useState<string | null>(null);

  // --- LÓGICA DE PROTEÇÃO E BUSCA ---
  useEffect(() => {
    let mounted = true;
    ...
    const secureFetch = async () => {
      try {
        ...
        // 3. AUTORIZADO! Libera a tela e busca dados
  ...
  const handleApproveSale = async (saleId: string) => {
    ...
    try {
      ...
      alert("Sucesso! Crédito aprovado.");
    } catch (error: any) {
  ...
  const handleDeleteSale = async (saleId: string) => {
    const confirmDelete = window.confirm("Tem certeza que deseja excluir esta transação? Isso não pode ser desfeito.");
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-50 gap-3">
        ...
        <p className="text-slate-500 font-medium text-sm animate-pulse">Verificando credenciais...</p>
      </div>
    );
  }

  // 2. Não Autorizado (Proteção Final)
