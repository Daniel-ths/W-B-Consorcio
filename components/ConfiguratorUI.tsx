...
interface ConfiguratorUIProps {
  currentCar: any;
  relatedCars: any[]; // outras versões do banco
  selectedColor: any;
  setSelectedColor: (v: any) => void;
  selectedWheel: any;
  setSelectedWheel: (v: any) => void;
  selectedSeatType: any;
  setSelectedSeatType: (v: any) => void;
  selectedAccs: string[];
...
export default function ConfiguratorUI({
  ...
  const [activeTab, setActiveTab] = useState<"Modelo" | "Exterior" | "Interior">(
    "Modelo"
  );
  ...
  const initialTrans =
    currentCar?.transmission_type === "manual" ? "Manual" : "Automático";
  const [transmissionFilter, setTransmissionFilter] = useState<
    "Automático" | "Manual"
  >(initialTrans);

  // --- 1) versões reais (carro atual + relacionados, sem duplicata) ---
  const allVersions = useMemo(() => {
    const all = [currentCar, ...(Array.isArray(relatedCars) ? relatedCars : [])]
      .filter(Boolean);

    const unique = all.filter(
      (car, index, self) =>
        index === self.findIndex((t) => toId(t?.id) === toId(car?.id))
    );

    return unique.sort(
      (a, b) => (Number(a?.price_start) || 0) - (Number(b?.price_start) || 0)
    );
  ...
  const filteredVersions = useMemo(() => {
    return allVersions.filter((car) => {
      ...
      return (
        (transmissionFilter === "Automático" && carTrans === "automatic") ||
        (transmissionFilter === "Manual" && carTrans === "manual")
      );
    });
  }, [allVersions, transmissionFilter]);

  // --- acessórios robustos (não assume array) ---
  const accessoriesArr = useMemo(
    () => (Array.isArray(currentCar?.accessories) ? currentCar.accessories : []),
    [currentCar]
  );

  const extAccessories = useMemo(
    () => accessoriesArr.filter((a: any) => a?.type === "exterior"),
    [accessoriesArr]
  );

  const intAccessories = useMemo(
    () => accessoriesArr.filter((a: any) => a?.type === "interior"),
    [accessoriesArr]
  );

  const isAccSelected = (accId: any) =>
    selectedAccs?.some((x) => toId(x) === toId(accId));
  ...
  const hasSeats =
    !!currentCar?.interior_images?.seats &&
    currentCar.interior_images.seats !== currentCar.interior_images.dash;

  const getTargetImageURL = () => {
    if (!currentCar) return "";

    if (activeTab === "Interior") {
      return interiorView === "seats"
        ? selectedSeatType?.image || currentCar?.interior_images?.seats
        : currentCar?.interior_images?.dash;
    }
  ...
  const handleSeatChange = (seat: any) => {
    if (toId(selectedSeatType?.id) !== toId(seat?.id)) {
  ...
  const rotateCar = (dir: "next" | "prev") => {
    if (dir === "next")
      setViewIndex((prev) => (prev + 1) % viewsOrder.length);
  ...
  const handleVersionClick = (carId: number) => {
    if (toId(carId) === toId(currentCar?.id)) return;
    router.push(`/configurador?id=${carId}`);
  };
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex flex-col lg:flex-row h-screen w-full overflow-hidden font-sans bg-white animate-in fade-in duration-500">
      ...
      <div className="lg:w-[75%] w-full h-[50vh] lg:h-full relative flex items-center justify-center overflow-hidden group">
        ...
        <div
          className={`absolute inset-0 z-50 bg-black/90 flex items-center justify-center transition-opacity duration-500 pointer-events-none ${
            isSwitchingCar ? "opacity-100" : "opacity-0"
          }`}
        >
        ...
        <button
          onClick={handleBack}
          className="absolute top-8 left-6 z-30 text-white/50 hover:text-white flex items-center gap-2 text-xs font-bold uppercase tracking-widest transition-all hover:-translate-x-1 hover:bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm"
        >
        ...
        <div
          className={`w-full h-full flex items-center justify-center relative transition-all duration-700 ease-in-out z-10 ${
            activeTab === "Interior" ? "p-0 scale-110" : "p-8 lg:p-24 scale-100"
          }`}
        >
          {/* Evita src="" */}
          <img
            ...
            alt="Car View"
            className={`absolute w-full h-full object-contain z-10 select-none pointer-events-none transition-all duration-700 ease-out ${
              isImageLoading ? "opacity-0 scale-105 blur-sm" : "opacity-100 scale-100 blur-0"
            }`}
          />
        ...
        {activeTab !== "Interior" && (
          <div className="absolute bottom-10 flex gap-4 z-30 transition-all duration-500 transform translate-y-4 opacity-0 group-hover:opacity-100 group-hover:translate-y-0">
            <button
              onClick={() => rotateCar("prev")}
              className="bg-white/10 hover:bg-white text-white hover:text-black p-4 rounded-full backdrop-blur-md transition-all shadow-lg border border-white/10"
            >
              <ChevronLeft size={24} />
            </button>
              ...
              {viewsOrder.map((_, i) => (
                <div
                  ...
                  className={`w-2 h-2 rounded-full transition-all duration-500 ${
                    i === viewIndex ? "bg-white w-6" : "bg-white/30"
                  }`}
                />
              ))}
            </div>
      ...
      <div className="lg:w-[25%] w-full h-full bg-white flex flex-col border-l border-gray-200 shadow-2xl z-30 animate-in slide-in-from-right duration-700">
          ...
          {user ? (
            <div className="flex items-center gap-2 mb-4 animate-in fade-in slide-in-from-top-2">
              ...
              <span className="text-xs font-bold text-green-700 uppercase tracking-wide">
                Vendedor: {(user.email || "").split("@")[0]}
              </span>
            </div>
          ) : (
            <span className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-4 block">
              Modo Visitante
            </span>
          )}
          ...
          <div className="flex justify-between mt-2 relative">
            {["Modelo", "Exterior", "Interior"].map((tab: any) => (
              <button
                ...
                className={`pb-3 text-sm font-bold uppercase tracking-wide transition-all duration-300 relative ${
                  activeTab === tab
                    ? "text-black"
              ...
              >
                {tab}
                {activeTab === tab && (
                  <span className="absolute bottom-0 left-0 w-full h-0.5 bg-black rounded-full animate-in zoom-in duration-300" />
                )}
              </button>
            ))}
          </div>
        ...
        <div
          ref={sidebarRef}
          className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar relative bg-gray-50/30"
        >
          <div key={activeTab} className="animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            {activeTab === "Modelo" && (
              <div>
                ...
                {/* lista versões filtradas */}
                <div className="space-y-3">
                  {filteredVersions.length > 0 ? (
                    filteredVersions.map((car, i) => (
                      <div
                        key={toId(car?.id) || i}
                        onClick={() => handleVersionClick(car.id)}
                        className={`cursor-pointer border-2 rounded-2xl p-5 transition-all duration-300 relative group overflow-hidden ${
                          toId(currentCar?.id) === toId(car?.id)
                      ...
                      >
                        <div className="flex justify-between items-center mb-1 relative z-10">
                          ...
                          {toId(currentCar?.id) === toId(car?.id) && (
                            <Check size={18} className="text-black bg-yellow-400 rounded-full p-0.5" />
                          )}
                        </div>
                        ...
                        <span className="text-sm font-bold text-gray-900 relative z-10">
                          {formatMoney(Number(car.price_start) || 0)}
                        </span>
                      </div>
                    ))
                  ) : (
              ...
              <div className="space-y-10">
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    {(Array.isArray(currentCar?.exterior_colors) ? currentCar.exterior_colors : []).map(
                      (c: any, i: number) => (
                        <button
                          key={c?.id || c?.name || i}
                          onClick={() => handleColorChange(c)}
                          className={`w-full aspect-square rounded-2xl shadow-sm transition-all duration-300 relative group overflow-hidden ${
                ...
                <div className="border-t border-gray-200 pt-8">
                  <h3 className="font-black uppercase text-xs tracking-widest text-gray-400 mb-4">
                    Rodas
                  </h3>

                  <div className="space-y-3">
                    {(Array.isArray(currentCar?.wheels) ? currentCar.wheels : []).map((w: any, i: number) => (
                      <div
                        key={toId(w?.id) || i}
                        onClick={() => setSelectedWheel(w)}
                        className={`flex items-center gap-4 p-2 pr-4 border-2 rounded-2xl cursor-pointer transition-all duration-200 ${
                          toId(selectedWheel?.id) === toId(w?.id)
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <span className="text-xs font-bold uppercase block text-gray-900 mb-0.5">
                            {w?.name || "Roda"}
                          </span>
                          <span className="text-[10px] font-bold text-gray-500">
                            {(w?.price || 0) === 0 ? "Série" : `+ ${formatMoney(Number(w?.price) || 0)}`}
                          </span>
                        </div>
                ...
                {/* ✅ ACESSÓRIOS (robusto: não some por não-array / id mismatch) */}
                <div className="border-t border-gray-200 pt-8">
                  <h3 className="font-black uppercase text-xs tracking-widest text-gray-400 mb-4">
                    Acessórios
                  </h3>
                  ...
                  <div className="space-y-3">
                    {extAccessories.length > 0 ? (
                      extAccessories.map((acc: any, i: number) => {
                        const selected = isAccSelected(acc?.id);
                        return (
                          <div
                            key={toId(acc?.id) || i}
                            onClick={() => toggleAccessory(toId(acc?.id))}
                            className={`flex gap-3 cursor-pointer p-2 border-2 rounded-2xl transition-all duration-200 ${
                              selected
                          ...
                          >
                            ...
                            <div className="flex-1 self-center min-w-0">
                              <p className={`text-xs font-bold uppercase truncate ${selected ? "text-white" : "text-gray-900"}`}>
                                {acc?.name || "Acessório"}
                              </p>
                              <p className={`text-[10px] font-bold ${selected ? "text-gray-300" : "text-gray-500"}`}>
                                {(acc?.price || 0) === 0 ? "Série" : `+ ${formatMoney(Number(acc?.price) || 0)}`}
                              </p>
                            </div>

                            <div
                              className={`w-6 h-6 rounded-full border-2 flex items-center justify-center self-center shrink-0 transition-colors ${
                      ...
                      <p className="text-xs text-gray-400 italic">
                        Nenhum acessório externo disponível.
                      </p>
                    )}

                    {/* opcional: mostrar internos também (pra não “sumir” caso estejam cadastrados) */}
                    {intAccessories.length > 0 && (
                      <div className="pt-3">
                        <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">
                          Interior
                        </p>
                        {intAccessories.map((acc: any, i: number) => {
                          const selected = isAccSelected(acc?.id);
                          return (
                            <div
                              key={toId(acc?.id) || i}
            ...
            {activeTab === "Interior" && (
              <div className="space-y-8">
                <div className="flex p-1 bg-gray-200 rounded-xl">
                  <button
                    onClick={() => handleInteriorViewChange("dash")}
                    className={`flex-1 py-3 px-3 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
                      interiorView === "dash"
                  ...
                  {hasSeats && (
                    <button
                      onClick={() => handleInteriorViewChange("seats")}
                      className={`flex-1 py-3 px-3 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all ${
                        interiorView === "seats"
                          ? "bg-white text-black shadow-md"
                          : "text-gray-500 hover:text-black"
                  ...
                  <div className="space-y-3">
                    <div
                      onClick={() => {
                        ...
                        handleInteriorViewChange("seats");
                      }}
                      className={`cursor-pointer border-2 rounded-2xl p-2 pr-4 flex gap-4 items-center transition-all ${
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-bold uppercase text-gray-900 mb-0.5">
                          Série
                        </p>
                    ...
                    {(Array.isArray(currentCar?.seat_types) ? currentCar.seat_types : []).map(
                      (s: any, i: number) => (
                        <div
                          key={toId(s?.id) || i}
                          onClick={() => handleSeatChange(s)}
                          className={`cursor-pointer border-2 rounded-2xl p-2 pr-4 flex gap-4 items-center transition-all ${
                            toId(selectedSeatType?.id) === toId(s?.id)
                              ? "border-black bg-white shadow-lg"
                              : "border-transparent bg-white hover:border-gray-200"
                          }`}
                        >
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-2xl font-black text-gray-900 leading-none tracking-tight">
                {formatMoney(Number(totalPrice) || 0)}
              </p>
            </div>
