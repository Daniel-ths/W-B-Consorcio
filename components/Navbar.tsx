...
/* NAVBAR BLINDADA (ANTI-LOOP)
   - Removemos qualquer dependência de 'router' dentro do useEffect.
   - Adicionamos verificação estrita de 'mounted' para evitar vazamento de memória.
   - O tratamento de erro é silencioso (não quebra a página).
*/
...
export default function Navbar() {
  ...
  // Estados de Perfil (Visual)
  const [fullName, setFullName] = useState("");
  const [avatarUrl, setAvatarUrl] = useState("");

  // --- O CORAÇÃO DO FIX (Lógica de Carregamento) ---
  useEffect(() => {
    let mounted = true;
    ...
    async function getProfileData(userId: string) {
      ...
      try {
        ...
        // Erro silencioso para não travar o site
        console.log("Navbar: Perfil não carregado (visitante ou erro)");
      }
    }

    async function initNavbar() {
      try {
        // 1. Pega a sessão atual sem forçar nada
    ...
    // 2. Escuta mudanças (Login/Logout) passivamente
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (!mounted) return;
      
      if (session?.user) {
        setUser(session.user);
        // Só busca o perfil se for um login novo para evitar chamadas duplas
        if (event === 'SIGNED_IN') {
           await getProfileData(session.user.id);
        }
      } else {
  ...
  }, []); // Array vazio OBRIGATÓRIO para não entrar em loop

  const handleLogout = async () => {
    await supabase.auth.signOut();
    // Refresh simples via window para limpar cache
      ...
      <nav className="fixed w-full z-[1001] top-0 bg-[#f8f8f8] border-b border-gray-200 h-16 flex items-center justify-between px-6 lg:px-12 shadow-sm font-sans">
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex gap-6 items-center">
            {/* Esconde o menu de veículos enquanto carrega para evitar layout shift */}
            {!loading && (
                <button 
        ...
        <div className="flex items-center gap-6 text-gray-600">
          ...
          {loading ? (
             // Placeholder enquanto carrega (evita piscar)
            ...
            <div className="relative group py-2">
              ...
              <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-gray-100 rounded-xl shadow-2xl p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform group-hover:translate-y-0 translate-y-2 origin-top-right">
                ...
                <div className="p-2 space-y-1">
                  {/* Link do Dashboard (Só aparece se tiver permissão) */}
                  {isAuthorized && (
