...
import { ChevronLeft, ChevronRight, Check, Loader2, ArrowLeft, ArrowRight, Info, Gauge, Armchair } from "lucide-react";
...
export default function VehicleConfigurator() {
  ...
  const [isInitialLoad, setIsInitialLoad] = useState(true); // Cortina inicial
  const [isSwitchingCar, setIsSwitchingCar] = useState(false); // Troca de versão
  const [imageLoaded, setImageLoaded] = useState(false); // Imagem do carro
  ...
  return (
    <>
      {/* --- CORTINA DE CARREGAMENTO (FADE OUT) --- */}
      <div 
        className={`fixed inset-0 z-[9999] bg-white flex items-center justify-center transition-opacity duration-1000 ease-in-out pointer-events-none ${isInitialLoad ? 'opacity-100' : 'opacity-0'}`}
      >
          ...
          showSummary ? (
            ...
            <div className="fixed inset-0 z-[2000] h-screen w-full overflow-y-auto font-sans bg-white animate-in fade-in slide-in-from-bottom-10 duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-1 p-1 bg-gray-100">
                    <div className="md:col-span-2 h-[50vh] bg-white relative overflow-hidden group"><div className="w-full h-full bg-center bg-contain bg-no-repeat transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${selectedColor?.images?.front || selectedColor?.image || currentCar.image_url})` }}></div></div>
                    <div className="h-[40vh] bg-white relative overflow-hidden group"><div className="w-full h-full bg-center bg-cover transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${selectedColor?.images?.rear || currentCar.image_back_url || currentCar.image_url})` }}></div></div>
                    <div className="h-[40vh] bg-white relative overflow-hidden group"><div className="w-full h-full bg-center bg-cover transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url(${selectedSeatType?.image || currentCar.interior_images?.dash})` }}></div></div>
                </div>
                <div className="max-w-5xl mx-auto px-8 py-16 space-y-20">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 mb-8 pb-4 border-b border-gray-200">Resumo</h2>
                        <div className="space-y-4">
                            <div className="flex justify-between font-bold"><span>Versão</span><span>{currentCar.model_name}</span></div>
                            <div className="flex justify-between"><span>Cor</span><span>{selectedColor?.name}</span></div>
                            <div className="flex justify-between border-t pt-4 text-xl font-bold"><span>Total</span><span>{formatMoney(totalPrice)}</span></div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="fixed inset-0 z-[2000] flex flex-col lg:flex-row h-screen w-full overflow-hidden font-sans bg-white">
                ...
                <div className={`w-full h-full flex items-center justify-center relative overflow-hidden transition-all duration-700 ease-in-out z-10 ${activeTab === 'Interior' ? 'p-0' : 'p-12 lg:p-32'}`}>
                    <div className={`relative w-full h-full flex items-center justify-center transition-all duration-700 ease-in-out transform drop-shadow-2xl ${activeTab === 'Interior' ? 'scale-105' : 'scale-100'}`}>
                        ...
                        <img 
                            ...
                            className={`absolute inset-0 w-full h-full object-contain z-10 select-none pointer-events-none transition-opacity duration-700 ease-in-out ${imageLoaded && !isSwitchingCar ? 'opacity-100' : 'opacity-0 scale-95'}`}
                        />
                    </div>
                </div>
              ...
              {/* --- DIREITA (CONFIGURAÇÃO) - SEM ANIMAÇÃO DE ENTRADA (Slide-in removido) --- */}
                ...
                <div ref={sidebarRef} key={activeTab} className="flex-1 overflow-y-auto p-6 pt-4 space-y-4 custom-scrollbar">
                    {activeTab === 'Modelo' && (
                        <div className="space-y-4">
                            ...
                            {/* Removido efeito cascata (stagger) e slide-in */}
                            {relatedCars.map((ver, index) => (
                                <Link 
                                    href={`/configurador?id=${ver.id}`} 
                                    key={ver.id} 
                                    onClick={(e) => {
                                        if(currentCar.id === ver.id) e.preventDefault();
                                    }}
                                    className={`block cursor-pointer border rounded-lg p-5 transition-all duration-200 relative group ${currentCar.id === ver.id ? 'border-black ring-1 ring-black bg-gray-50' : 'border-gray-200 hover:border-gray-400'}`}
                                >
                            ...
                            <div className="border-t border-gray-100 pt-8">
                                ...
                                <div className="space-y-4">
                                    {extAccessories.length > 0 ? extAccessories.map((acc: any, index: number) => (
                                        <div key={acc.id} onClick={() => toggleAccessory(acc.id)} className={`flex gap-4 cursor-pointer p-3 rounded hover:bg-gray-50 transition-all duration-200 ${selectedAccs.includes(acc.id) ? 'bg-gray-50 ring-1 ring-gray-200' : ''}`}>
                    ...
                    {activeTab === 'Interior' && (
                        <div className="space-y-8 pt-2">
                            ...
                            <div className="border-t border-gray-100 pt-6">
                                ...
                                <div className="space-y-4">
                                    <div onClick={() => { setSelectedSeatType(null); updateInteriorImage('seats'); }} className={`cursor-pointer border rounded-lg p-3 flex gap-4 items-center transition-all ${selectedSeatType === null ? 'border-black ring-1 ring-black' : 'border-gray-200 hover:border-gray-300'}`}>
                                        <div className="w-20 h-20 bg-white rounded flex items-center justify-center border border-gray-100 shrink-0 overflow-hidden"><img src={currentCar.interior_images?.seats || currentCar.interior_images?.dash} className="w-full h-full object-cover" /></div>
                                        <div className="flex-1 min-w-0"><p className="text-sm font-bold text-gray-900 leading-tight mb-2">{currentCar.interior_images?.seats_desc || 'Bancos Padrão'}</p><p className="text-xs font-bold text-gray-500">Incluso</p></div>
